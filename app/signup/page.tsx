...
// ═══════════════════════════════════════════════════════════════
// OTP VERIFICATION COMPONENT (reused in signup and login)
// ═══════════════════════════════════════════════════════════════
...
export default function SignupPage() {
  ...
  const canSubmit = fullName && emailValid && password.length >= 8 && termsAccepted
  ...
  if (showVerification) {
    return (
      <OtpVerificationScreen
        email={email}
        type="signup"
        onBack={() => { setShowVerification(false); setErrorMessage('') }}
  ...
  return (
    <div className="min-h-screen bg-[#050505] flex">
      ...
      <div className="hidden lg:flex lg:w-1/2 relative overflow-hidden">
        {/* Ambient glows */}
        <div className="absolute bottom-1/4 left-1/3 w-[500px] h-[500px] rounded-full bg-emerald-500/10 blur-[150px] animate-pulse" />
        <div className="absolute top-1/3 right-1/4 w-[400px] h-[400px] rounded-full bg-cyan-500/8 blur-[120px] animate-pulse" style={{ animationDelay: "1s" }} />
        <div className="absolute bottom-1/3 left-1/4 w-[300px] h-[300px] rounded-full bg-purple-500/6 blur-[100px] animate-pulse" style={{ animationDelay: "2s" }} />
        ...
        <div className="relative z-10 flex flex-col justify-between p-12 xl:p-16 h-full">
          ...
          <div className="max-w-md">
            ...
            <p className="text-muted-foreground">
              — Join 12,500+ members building the future
            </p>
          </div>

          {/* Credibility */}
          <p className="text-sm text-muted-foreground">
            ₹200Cr+ raised through our platform
          </p>
        </div>
      </div>
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-6 md:p-12 relative overflow-y-auto">
        ...
        >
          {/* Mobile logo */}
          <motion.div variants={itemVariants} className="lg:hidden mb-6 text-center flex flex-col items-center">
            ...
            <p className="text-sm text-muted-foreground">
              Join 12,500+ founders and investors
            </p>
          </motion.div>
          ...
          <motion.div variants={itemVariants} className="mb-6">
            ...
            <p className="text-muted-foreground">
              Start your journey to the next big thing
            </p>
          </motion.div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <motion.div variants={itemVariants}>
              ...
              <div className="relative">
                <input
                  ...
                  className={`w-full h-12 px-4 pr-12 rounded-xl bg-white/5 border text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 transition-all ${emailTouched && email
                    ? emailValid
                ...
                />
                {emailTouched && email && (
                  <div className={`absolute right-4 top-1/2 -translate-y-1/2 ${emailValid ? "text-emerald-400" : "text-red-400"}`}>
                    {emailValid ? (
              ...
              {emailTouched && email && emailValid && (
                <p className="mt-1.5 text-xs text-emerald-400">Looks good!</p>
              )}
            </motion.div>
            ...
            <motion.div variants={itemVariants} className="flex items-start gap-2 pt-2">
              ...
              <label htmlFor="terms" className="text-sm text-muted-foreground cursor-pointer leading-relaxed">
                I agree to the{" "}
                <a href="/terms" target="_blank" rel="noopener noreferrer" className="text-emerald-400 hover:text-emerald-300 transition-colors">Terms of Service</a>
                {" "}and{" "}
                <a href="/privacy" target="_blank" rel="noopener noreferrer" className="text-emerald-400 hover:text-emerald-300 transition-colors">Privacy Policy</a>
              </label>
            </motion.div>
            ...
            <motion.div variants={itemVariants} className="pt-2">
              <Button
                ...
                className="w-full h-12 rounded-xl bg-emerald-500 hover:bg-emerald-600 text-black font-semibold transition-all hover:scale-[1.02] active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100 glow-emerald-sm"
              >
                {isLoading ? (
                  <>
